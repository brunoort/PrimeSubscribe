...
app.get('/subscription/:cpf', function (req, res) {
    ...
    if (reqHeader.wctoken !== "" && reqHeader.wctoken !== null) {
        ...
        getCustomerInfo(reqHeader.wctoken, reqHeader.wctrustedtoken)
        .then(res => {
            console.log('getCustomerInfo', res);
            customerInfoCPF = res.cpf;
        }).then(() => {

            console.log('customerInfoCPF', customerInfoCPF);
            console.log('customerCPF', customerCPF);

            if (customerInfoCPF == customerCPF) {
                        ...
                        if (customerCPF === customerRegistryCode) {
                            ...
                            getCustomerSubsById(customerId)
                                .then(customerRes => {
                                    ...
                                    //console.log("subs",customerRes.subscriptions);
                                    ...
                                    if (customerRes.subscriptions.length > 0) {
                                        customerRes.subscriptions.forEach(subs => {
                                            if (subs.status === 'active') {
                                                ...
                                                //    data: null
                                                //}));
                                                //fabrizio 15/10 tratar inativo para status cancelado
                                                //retirado subs.status = 'inactive'
                                        ...
                                        .then(() => {
                                            ...
                                            getCustomerInstallations(reqHeader.wctoken, reqHeader.wctrustedtoken, customerCPF)
                                                ...
                                                .then((response => {
                                                    ...
                                                    if (response.subscription.plan !== null && response.subscription.plan !== undefined) {

                                                        var membershipBody = {
                                                            "memberships": [{
                                                                "code": response.subscription.plan.id,
                                                                "class": response.subscription.plan.name,
                                                                "type": response.subscription.plan.name
                                                            }]
                                                        };

                                                        getMembership(reqHeader.wctoken, reqHeader.wctrustedtoken, membershipBody)
                                                            .then(() => {
                ...
                res.status(400).end(JSON.stringify({
                    message: 'Não foi possível recuperar sua assinatura, CPF ou Token inválidos',
                    statusCode: 120,
                    data: null
                }));
            }
...
function formatingResponse(customerResponse, subsResponse, billsResponse, installationResponse, beneficitsResponse) {
    ...
    if (subsResponse.current_period !== null && subsResponse.current_period !== undefined) {
        var mDataVigencia = new Date(subsResponse.current_period.end_at);
    }

    //console.log('paymentProfile', subsResponse.payment_profile);

    if (subsResponse.status === 'active' && subsResponse.payment_profile !== null) {
    ...
    } else if (mDataVigencia > mData && (mDataVigencia !== undefined || mDataVigencia && mDataVigencia !== null)) {
...
function createSubsBody(subsResponse) {
    try {
        ...
        //fabrizio 23/10 tratar payment_profile null para status cancelado
        var mData = new Date();

        if (subsResponse.current_period !== null && subsResponse.current_period !== undefined) {
            var mDataVigencia = new Date(subsResponse.current_period.end_at);
            console.log('data vigencia', mDataVigencia);
        }

        var flagDtendGeDtatual = false;
        var flagDtcancelGeDtstart = false;
        var mDataCancelamento = new Date(subsResponse.cancel_at);
        var mDtcancelGeDtstart = new Date(subsResponse.start_at);
        var mDataInicioem = mDtcancelGeDtstart.setDate(mDtcancelGeDtstart.getDate() + 7);
        mDataInicioem = new Date(mDataInicioem);


        // console.log('data cancelamento', mDataCancelamento);
        // console.log('data incio', mDtcancelGeDtstart);
        // console.log('data incio mais 7', mDataInicioem);
        ...
        if (mDataVigencia !== undefined) {
            if (mDataVigencia > mData) {
                ...
                flagDtendGeDtatual = true;
            }
        } else {
            flagDtcancelGeDtstart = false;
        }

        if (mDataCancelamento > mDataInicioem || (subsResponse.current_period === null || subsResponse.current_period === undefined)) {
            flagDtcancelGeDtstart = true;
        }

        return {
            id: subsResponse.id,
            status: convertStatus(subsResponse.status, subsResponse.overdue_since, flagDtendGeDtatual, flagDtcancelGeDtstart),
...
function convertStatus(status, overdue, flgdtendatual, flgdtcancelstart) {

    //console.log(overdue);

    switch (status) {
        case 'active':
            if (overdue === null || overdue === '' && overdue !== undefined) {
                return 'active';
            } else {
                return 'inactive';
            }
        ...
        case 'canceled':
            if (overdue === null || overdue === '') {
                if (flgdtendatual === true && flgdtcancelstart === true) {
                    return 'active';
                }
                if (flgdtendatual === false && flgdtcancelstart === true) {
                    return 'inactive';
                }
...
function getMembership(vWcToken, vWcTrustedToken, body) {
    return new Promise((resolve, reject) => {
        const url = 'https://' + domainFast + '/v1/checkout/cart/memberships'
        var options = {
            uri: url,
            method: 'POST',
            body: body,
